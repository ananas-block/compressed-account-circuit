...
describe('CompressedAccount Circuit Test', function() {
  ...
  it('should correctly hash compressed account data with poseidon and test the circuit', async function() {
    ...
    const hardcodedInputs = [
      ...
      "100", // lamports
      "38795127103506095060616951389784305450515281095163674791955527140240872547", // address
      "1", // discriminator
    ...
    const hardcodedHashInputs = [
      ...
      "1", // leaf_index
      "274698893731942391252194363483372283063414035850745528064643496257815950661", // merkle_tree_hashed
      "18446744073709551716", // lamports
      "38795127103506095060616951389784305450515281095163674791955527140240872547", // discriminator
      "36893488147419103233", // has_data
      "3459342586129817037903285055794736781430278749885408056232073615594126477392" // data_hash
    ];
    ...
    console.log("lamports:", inputFields[3].toString());
    console.log("discriminator:", inputFields[4].toString());
    console.log("has_data:", inputFields[5].toString());
    console.log("data_hash:", inputFields[6].toString());
    ...
    const poseidonInputs = hardcodedHashInputs.map(x => poseidon.F.e(x.toString()));
    ...
    try {
      await exec('cd ' + path.join(__dirname, '..') + ' && circom circuits/compressed_account.circom --r1cs --wasm --sym');
    ...
    try {
      await exec('cd ' + path.join(__dirname, '..') + ' && node_modules/.bin/snarkjs wtns calculate compressed_account_js/compressed_account.wasm compressed_account_input.json witness.wtns');
